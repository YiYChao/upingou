...
app.controller('goodsController' ,function($scope,$controller,$location,goodsService,uploadService,itemCatService,typeTemplateService){	
	...
	$scope.uploadImage = function(){
		uploadService.uploadeImage().success(
			function(response){
				if(response.success){
	...
	$scope.$watch("entity.goods.typeTemplateId",function(newValue, oldValue){
		typeTemplateService.findOne(newValue).success(
			function(resposne){
				$scope.typeTemplate=resposne; //获取类型模板
	...
	$scope.createItemList = function(){
		$scope.entity.itemList=[{spec:{},price:0,num:999,status:'0',isDefault:'0' } ];//列表初始
	...
	// 添加列的值
	addColumn = function(list,columnName,colunmValues){
		var newList = [];//新的集合
		for(var i = 0; i < list.length; i++){
			var oldRow = list[i];
			for(var j = 0; j < colunmValues.length; j++){
				var newRow = JSON.parse(JSON.stringify(oldRow));	// 深克隆
				newRow.spec[columnName] = colunmValues[j];
				newList.push(newRow);
			}
		}
		return newList;
